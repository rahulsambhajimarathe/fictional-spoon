...
class AdminNotifications extends Controller {
    ...
    public function index() {
        ...
        $filters->set_default_order_by('notification_id', $this->user->preferences->default_order_type ?? settings()->main->default_order_type);
            ...
            switch($_POST['type']) {
                case 'delete':
                    ...
                    foreach($_POST['selected'] as $notification_id) {
                        $user_id = db()->where('notification_id', $notification_id)->getValue('notifications', 'user_id');

                        /* Delete the notification */
                        db()->where('notification_id', $notification_id)->delete('notifications');

                        /* Clear the cache */
                        cache()->deleteItemsByTag('notification_id=' . $notification_id);
